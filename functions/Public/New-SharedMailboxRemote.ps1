...
function _GetExchangePSSession {
    ...
    try {
        ...
        if (-not (Test-Path -Path $CredentialPath)) {
            ...
            Write-Warning "Run Initialize-ScheduledTaskCredential to create credential file"
            return $null
        }
...
function _AddToMailboxProvisioningBacklog {
    ...
    $newEntry = [PSCustomObject]@{
        ...
        adminGroup = if ([string]::IsNullOrWhiteSpace($AdminGroupName)) { $null } else { $AdminGroupName }
